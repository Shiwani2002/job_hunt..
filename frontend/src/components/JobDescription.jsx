  ...
  return (
    <div className="max-w-7xl mx-auto my-10">
      <div className="flex items-center justify-between">
        ...
        {/* Apply Now button with only link functionality */}
        <Button
          onClick={() => {
            const jobLink = singleJob?.description || '';
            if (jobLink) {
              window.open(jobLink, '_blank'); // Open job link in a new tab
            } else {
              toast.error('Job link is not available.');
            }
          }}
      ...
      {/* New Button for Apply Status */}
      <div className="mt-4">
        <Button
          onClick={isApplied ? null : applyJobHandler}
          disabled={isApplied}
          className={`rounded-lg ${
            isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'
          }`}
        >
          {isApplied ? 'Already Applied' : 'I have applied'}
        </Button>
      </div>

      {/* New Button to navigate to JobRequirements */}
      <div className="mt-4">
      ...
      <div className="my-4">
        ...
        <h1 className="font-bold my-1">
          Job_Link: <span className="pl-4 font-normal text-gray-800">{singleJob?.description}</span>
        </h1>
        <h1 className="font-bold my-1">
